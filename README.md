...
└── api/
    ├── assessment/[id]/report/   PDF generation
...
| `CLERK_WEBHOOK_SECRET` | Clerk dashboard → Webhooks | ⚠️ | Reserved for future user-sync webhook endpoint |
...
5. **(Future)** Configure a webhook in Clerk → Webhooks pointing at `https://<your-domain>/api/webhooks/clerk` and paste the signing secret into `CLERK_WEBHOOK_SECRET`. The webhook handler is not yet implemented.
